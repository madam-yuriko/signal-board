...
function FilterTagGroup({ label, allLabel = "すべて", options, selected, onSelect }: {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <span className="mr-1 text-[10px] font-semibold text-gray-500">{label}</span>
      {[{ value: "all", label: allLabel }, ...options].map((option) => {
        const active = selected === option.value;
        return <button key={option.value} type="button" onClick={() => onSelect(option.value)} aria-pressed={active} className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold transition-colors ${active ? "border-cyan-400/45 bg-cyan-400/15 text-cyan-100" : "border-white/10 bg-black/15 text-gray-500 hover:border-white/20 hover:text-gray-200"}`}>{option.label}</button>;
      })}
    </div>
  );
}
    ...
    <article className="glass-card group overflow-hidden rounded-xl">
      ...
      <div className="p-3.5">
        ...
        <div className="mt-3 border-t border-white/7 pt-3">
          ...
          {record.taxonomyGroup === "未分類" && <button type="button" onClick={() => onEdit(record)} className="mt-2 inline-flex items-center gap-1 rounded-md border border-amber-300/25 bg-amber-300/8 px-2 py-1 text-[10px] font-semibold text-amber-100 hover:bg-amber-300/14">Wikipedia名を指定して再取得 <Pencil className="h-3 w-3" /></button>}
        </div>
      ...
      <div className="my-auto w-full max-w-2xl rounded-xl border border-white/10 bg-[#12141a] shadow-2xl">
        ...
        <form onSubmit={submit} className="p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {showWikipediaNameField && <div className="sm:col-span-2 rounded-lg border border-amber-300/15 bg-amber-300/[0.04] p-3"><p className="text-[11px] font-semibold text-amber-100">Wikipedia名を指定して再取得</p><p className="mt-1 text-[10px] leading-relaxed text-gray-500">店頭名は変えず、Wikipediaのページ名だけを指定して、分類・学名・飼育メモ・写真を自動で取り直します。</p><div className="mt-3"><Field label="Wikipedia名" hint="日本語Wikipediaの記事名"><input name="wikipediaName" defaultValue={record?.wikipediaName} placeholder="例：ネオンテトラ" className={inputClass} /></Field></div></div>}
...
export default function AquariumDashboard({ initialRecords }: { initialRecords: AquariumRecord[] }) {
  ...
  const filtered = useMemo(() => records.filter((record) => {
    ...
    return (group === "all" || record.taxonomyGroup === group) && tankMatches && (survival !== "alive" || !record.deathDate) && (!query.trim() || haystack.includes(query.trim().toLocaleLowerCase("ja")));
  }), [group, query, records, survival, tank]);
  ...
  return <div className="space-y-4">
    ...
    <section aria-label="記録を絞り込む" className="glass-card space-y-2.5 rounded-lg px-3 py-2.5">
