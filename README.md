...
ボクシングモバイルは同じURLに対しておよそ3回に1回しか本来のページを返さず、残りは無関係な広告ページを200で返します。取得側でこれを検出して最大10回までリトライし、広告ページはキャッシュしません。
JBC側が仮名称で持っている地方興行も、開催日と会場が一致すればボクシングモバイルのカードと突き合わせます。
...
選手別結果・予定の一覧は、日本語版Wikipediaにその選手の記事と戦績表がある場合のみWikipediaを正本とし、通算戦績（◯戦◯勝◯KO◯敗◯分）を一覧の上に表示します。会場・階級・興行名は本アプリの収録興行から補完し、Wikipediaに未反映の今後の試合予定は収録データから追加します。記事や戦績表が無い選手は従来どおり収録データだけを表示します。取得結果は選手ごとに1日キャッシュします。
